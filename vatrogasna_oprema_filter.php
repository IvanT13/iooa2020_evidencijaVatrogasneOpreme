        ...
        <div class="row">
            <div class="col">
                ...
                require('connection.php');
                    ...
                    $naziv_lokacija = $_POST['lokacija'];
                    $sql = "SELECT * FROM lokacije";
                    $myData = mysqli_query($con,$sql); //pull podataka iz baze
                    while($record = mysqli_fetch_array($myData)){
                        //Prebacivanje teksta u ID
                        if(strcmp($record['naziv_lokacije'],$naziv_lokacija) == 0){
                        $lokacija = $record['ID'];
                        }        
                    } 
                    $sql = "SELECT * FROM vatrogasna_oprema WHERE lokacija='$lokacija'";
                    
                    $myData = mysqli_query($con,$sql); //pull podataka iz baze opreme
                    //ispis podataka
                    echo '<table class="table">
                        <thead>
                        <th colspan="5">Oprema zadužena na: ' . $naziv_lokacija . '</th>
                        </thead>
                        <thead>
                            <tr>
